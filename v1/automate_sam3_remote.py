...
def resolve_config(args) -> dict:
    ...
    required_by_command = {
        "verify": ["password"],
        "setup": ["password", "drive_folder_id", "drive_folder_url"],
        "upload-pipeline": ["password"],
        "launch": ["password"],
        "status": ["password"],
        "samtop": ["password"],
        "full": ["password", "drive_folder_id", "drive_folder_url"],
    }
    ...
    if not snapshot.get("session_exists"):
        ...
        for worker_name in ["worker_a", "worker_b"]:
            ...
            if item is not None:
                ...
                lines.append(
                    f"  prompt={shorten(current_prompt_label(item), 28)}  "
        ...
        if interesting:
            ...
            for item in interesting:
                ...
                lines.append(
                    ...
                    f"{chunk_done}/{chunk_total or '?'} chunks"
                )
                if detail:
